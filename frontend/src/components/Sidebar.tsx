    ...
    <div className="w-72 bg-gray-900 border-l border-gray-700 p-4 overflow-y-auto flex flex-col gap-4">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <label className="text-xs text-gray-400">图案设置</label>
          <button onClick={() => store.resetGroup('pattern')}
            className="text-xs text-indigo-400 hover:text-indigo-300">↺ 重置图案</button>
        </div>
        <div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <label className="text-xs text-gray-400">配色设置</label>
          <button onClick={() => store.resetGroup('color')}
            className="text-xs text-indigo-400 hover:text-indigo-300">↺ 重置配色</button>
        </div>
        <div>
        ...
        <div>
          ...
          <input type="color" value={store.bgColor}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <label className="text-xs text-gray-400">导出设置</label>
          <button onClick={() => store.resetGroup('export')}
            className="text-xs text-indigo-400 hover:text-indigo-300">↺ 重置导出</button>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="text-xs text-gray-400">宽度: {store.width}</label>
            <input type="range" min={400} max={1600} step={10} value={store.width}
              onChange={e => store.setParam('width', Number(e.target.value))} className="w-full accent-teal-500" />
          </div>
          <div>
            <label className="text-xs text-gray-400">高度: {store.height}</label>
            <input type="range" min={400} max={2000} step={10} value={store.height}
              onChange={e => store.setParam('height', Number(e.target.value))} className="w-full accent-rose-500" />
          </div>
        </div>
